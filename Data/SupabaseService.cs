...
public class SupabaseService : ISupabaseService, IDisposable, IAsyncDisposable
{
    ...
    {
        var url = configuration["Supabase:Url"] ?? SupabaseConfig.Url;
        var key = configuration["Supabase:AnonKey"] ?? SupabaseConfig.AnonKey;
        var serviceKey = configuration["Supabase:ServiceRoleKey"] ?? SupabaseConfig.ServiceRoleKey;
    ...
    public async Task MarkMessagesAsReadAsync(Guid conversationId, Guid userId)
    {
        var messages = await _client.From<Message>()
            ...
            .Filter("read_at", Operator.Equals, (object?)null)
            .Get();
